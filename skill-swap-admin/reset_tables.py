    ...
    flagged = [
        {
            ...
            "name": row[1],
            "description": row[2],
            "ai_moderation_status": row[3],
            "ai_moderation_reason": row[4],
            "reviewed_by_admin": row[5]
        }
...
def get_flagged_reviews():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, reviewer_user_id, reviewed_user_id, message, ai_moderation_status, ai_moderation_reason, reviewed_by_admin
        FROM reviews
        WHERE ai_moderation_status='rejected' AND reviewed_by_admin=FALSE
    """)
    flagged = [
        {
            "id": row[0],
            "reviewer_user_id": row[1],
            "reviewed_user_id": row[2],
            "message": row[3],
...
# ========== ADMIN SEND PLATFORM MESSAGE TO USER ==========

@app.post("/admin/message")
def send_admin_message(data: dict = Body(...)):
    to_user_id = data.get("to_user_id")
    message = data.get("message", "")
    if not to_user_id or not message.strip():
        raise HTTPException(status_code=400, detail="Invalid input.")
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute(
            "INSERT INTO admin_messages (to_user_id, message) VALUES (%s, %s)",
            (to_user_id, message)
        )
        conn.commit()
    except Exception as e:
...
def download_user_report(user_id: int):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT u.id, u.name, u.email, u.is_banned, u.ban_reason,
